...
import { GIGS } from "@/lib";
import { sortGigsByDate, filterUpcomingGigs, filterPassedGigs } from "@/lib/utils";
...
const FacebookIcon = () => (
  ...
  >
    ...
    <g>
      <path
        style={{ fill: "#FFFFFF" }}
        d="M363.043,109.466H148.958c-21.809,0-39.49,17.68-39.49,39.49v214.085c0,21.811,17.68,39.49,39.49,39.49h105.584l0.183-104.722h-27.21c-3.536,0-6.406-2.86-6.418-6.396l-0.133-33.759c-0.014-3.553,2.867-6.444,6.42-6.444h27.162v-32.618c0-37.852,23.118-58.463,56.884-58.463h27.71c3.543,0,6.42,2.874,6.42,6.42v28.463c0,3.546-2.874,6.42-6.416,6.42l-17.006,0.01c-18.363,0-21.921,8.725-21.921,21.533v28.239h40.351c3.848,0,6.83,3.358,6.375,7.173l-4.001,33.759c-0.381,3.232-3.122,5.665-6.375,5.665h-36.168l-0.183,104.726h62.826c21.809,0,39.49-17.682,39.49-39.491v-214.09C402.533,127.147,384.852,109.466,363.043,109.466L363.043,109.466z"
      />
      <polygon
        style={{ fill: "#FFFFFF" }}
    ...
    <path
      style={{ fill: "#D1D1D1" }}
      d="M363.043,109.466H254.277v141.741h0.269V218.59c0-37.852,23.118-58.463,56.884-58.463h27.71c3.543,0,6.42,2.874,6.42,6.42v28.463c0,3.546-2.874,6.42-6.416,6.42l-17.006,0.01c-18.363,0-21.921,8.725-21.921,21.533v28.238h40.351c3.848,0,6.83,3.358,6.375,7.173l-4.001,33.759c-0.381,3.232-3.122,5.665-6.375,5.665h-36.168l-0.183,104.726h62.826c21.809,0,39.49-17.682,39.49-39.491V148.956C402.533,127.147,384.852,109.466,363.043,109.466z"
    />
  </svg>
);

const GigRow = ({ gig, isUpcoming }: { gig: any; isUpcoming: boolean }) => (
  <tr className="pt-8 text-xs text-white lg:text-xl 2xl:text-2xl">
    ...
    <td className="text-sm text-white sm:text-center lg:text-lg 2xl:text-2xl">
      {gig.info ? (
        <SocialLink
          ...
          ariaLabel={`More info about ${gig.name}`}
        >
          {gig.name}
        ...
        <>
          <span className="cursor-not-allowed">{gig.name}</span>
    ...
    <td>
      {gig.link && (
        <SocialLink href={gig.link} ariaLabel={`Link to ${gig.name} event`}>
          <FacebookIcon />
        </SocialLink>
      )}
    </td>
...
export default function Gigs() {
  ...
  return (
    <>
      <SectionHeader right={true} center={true} subTitle="Gigs" />
          ...
          <table className="w-full border-separate lg:border-spacing-4 table-auto">
            ...
            <tbody>
              {upcomingGigs.map((gig, i) => (
                <GigRow key={`upcoming-${i}`} gig={gig} isUpcoming={true} />
              ))}
            </tbody>
            <thead className="flex justify-start sm:justify-center lg:text-xl 2xl:text-2xl font-bold">
              <tr className="flex">
            ...
            <tbody>
              {passedGigs.map((gig, i) => (
                <GigRow key={`passed-${i}`} gig={gig} isUpcoming={false} />
              ))}
            </tbody>
          </table>
        </div>
