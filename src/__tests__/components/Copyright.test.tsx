...
describe('Copyright', () => {
    ...
    it('renders logo with click handler', () => {
        ...
        expect(logoContainer).toHaveClass('cursor-pointer', 'transition-all', 'fill-white', 'hover:fill-bubblegum', 'ease-in-out');
    });
    ...
    it('has correct container structure and classes', () => {
        ...
        expect(mainDiv).toHaveClass('relative', 'z-3', 'grid', 'lg:grid-cols-2', 'gap-4', 'p-8', 'bg-black/90', 'backdrop-blur-md');
    });

    it('has correct copyright section classes', () => {
        render(<Copyright />);

        const copyrightSection = screen.getByText('Austin Dest').closest('div')?.parentElement;
        expect(copyrightSection).toHaveClass('flex-col', 'justify-center', 'flex', 'items-center');
    });

    it('calls scrollToTop when logo is clicked', () => {
