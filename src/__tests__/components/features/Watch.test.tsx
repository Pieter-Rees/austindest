...
jest.mock('@/components/ui/SectionHeader', () => ({
    SectionHeader: ({ subTitle, left }: any) => (
        <div data-testid="section-header">
            <h2>{subTitle}</h2>
            <span data-testid="section-header-props">left:{left ? 'true' : 'false'}</span>
        </div>
    ),
}));
...
describe('Watch', () => {
    it('renders section header with correct props', () => {
        ...
        expect(screen.getByTestId('section-header-props')).toHaveTextContent('left:true');
    });
    ...
    it('renders video player with correct props', () => {
        ...
        expect(player).toHaveAttribute('data-src', 'https://youtube.com/watch?v=example');
    });
