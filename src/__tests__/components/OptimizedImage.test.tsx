...
jest.mock('next/image', () => ({
    __esModule: true,
    default: ({ src, alt, width, height, className, priority, quality, placeholder, blurDataURL, onLoad, onError, sizes }: any) => (
        <img
...
describe('OptimizedImage', () => {
    ...
    it('renders loading placeholder initially', () => {
        ...
        expect(loadingDiv).toHaveClass('absolute', 'inset-0', 'bg-gray-800', 'animate-pulse', 'rounded-3xl');
    });
    ...
    it('applies correct classes to image', () => {
        ...
        const image = screen.getByTestId('next-image');
        expect(image).toHaveClass('object-cover', 'w-full', 'h-full', 'rounded-3xl', 'transition-opacity', 'duration-300', 'opacity-0');
    });
