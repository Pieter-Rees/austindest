...
jest.mock('@/components/ui/title', () => ({
    Title: ({ title, subTitle, margin, left, center }: any) => (
        <div data-testid="title">
            {title && <h1>{title}</h1>}
            {subTitle && <h2>{subTitle}</h2>}
            <span data-testid="props">
                margin:{margin ? 'true' : 'false'} left:{left ? 'true' : 'false'} center:{center ? 'true' : 'false'}
            </span>
        </div>
    )
}));
...
describe('Landing', () => {
    it('renders title with correct props', () => {
        ...
        expect(screen.getByTestId('props')).toHaveTextContent('margin:true left:true center:true');
    });
    ...
    it('has proper container structure', () => {
        ...
        expect(container).toHaveClass('w-full', 'h-full', 'my-6', 'lg:my-0', 'relative', 'z-2');
    });
    ...
    it('has proper socials container', () => {
        ...
        expect(socialsContainer).toHaveClass('flex', 'justify-center', 'lg:justify-end', 'mt-16');
    });
