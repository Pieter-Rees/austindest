...
jest.mock('@/components/ui/SocialLink', () => ({
    SocialLink: ({ href, children, className, ariaLabel }: any) => (
        <a href={href} className={className} aria-label={ariaLabel} data-testid="social-link">
            {children}
        </a>
    )
}));
...
describe('Contact', () => {
    it('renders section header with correct props', () => {
        ...
        expect(screen.getByTestId('props')).toHaveTextContent('right:false center:true');
    });
    ...
    it('renders Instagram handle link', () => {
        ...
        expect(instagramLink).toHaveAttribute('href', 'https://www.instagram.com/austindestmusic/');
